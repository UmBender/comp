...
bool test(vector<int> &arr, string &s) {
  if (arr.size() != s.size()) {
    return false;
  }
  unordered_map<int, int> to_s;
  unordered_map<int, int> to_arr;

  for (int i = 0; i < arr.size(); i++) {
    if (to_s.find(arr[i]) == to_s.end()) {
      if (to_arr.find(s[i]) != to_arr.end()) {
        return false;
      } else {
        to_s[arr[i]] = s[i];
        to_arr[s[i]] = arr[i];
      ...
      if (to_arr.find(s[i]) == to_arr.end()) {
        ...
        if (to_s[arr[i]] != s[i] || to_arr[s[i]] != arr[i]) {
          return false;
        }
      }
    }
