...
void solution() {
  ...
  for (int i = 0; i < h; i++) {
    for (int j = 0; j < l; j++) {
      // cerr << "i: " << i << " j: " << j;
      grid[i * l + j] = min(i + 1, min(k, h - k + 1), h - i) *
                        min(j + 1, min(k, l - k + 1), l - j);
      // cerr << " min(i): " << min(i + 1, min(k, h - k + 1), h - i);
      // cerr << " min(j): " << min(j + 1, min(k, l - k + 1), l - j);
      // cerr << " grid: " << grid[i * l + j] << endl;
    }
  }
  sort(grid.rbegin(), grid.rend());
  int counter{0};
  for (int i = 0; i < a.size(); i++) {
    // cerr << "Grid: " << grid[i] << " Monkey height: " << a[i] << endl;
    counter += grid[i] * a[i];
  }
  cout << counter << endl;
}
