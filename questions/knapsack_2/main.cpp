...
void solution() {
  ...
  for (auto &i : wv) {
    ...
    max_vi = max(i.second, max_vi);
  }
  sort(wv.begin(), wv.end());
  vector<int> dp(n * max_vi + 1, INT_MAX);
  int max_val = 0, max_val_pos = 0;
  ...
  for (int i = 0; i < n; i++) {
    for (int j = dp.size() - 1; j >= 0; j--) {
      ...
      if (j == wv[i].second) {
        dp[j] = min(dp[j], wv[i].first);
      } else if (dp[j - wv[i].second] != INT_MAX) {
        dp[j] = min(dp[j], dp[j - wv[i].second] + wv[i].first);
      }
      if (max_val < j && dp[j] <= mw) {
        max_val = j;
      }
    }
  }
